=== SKT Ayurveda ===

Contributors: sonalsinha21
Requires PHP:  5.6
Tested up to: 6.9
Version: 2.1
License: GNU General Public License v2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html
Tags:two-columns,right-sidebar,full-width-template,custom-background,custom-colors,custom-menu,sticky-post,theme-options,threaded-comments,featured-images,footer-widgets,translation-ready,rtl-language-support,custom-logo,editor-style,custom-header,blog,e-commerce,portfolio

== Description ==

Ayurvedic Medicine WordPress theme for ayurveda, herbal, body clinic, skin, spa treatment, health, wellness, naturopathy, homeopathy, healing, spa, massage, reiki, yoga, meditative practice, exercise, stretching, discipline, physical, spiritual, relaxing help, green, nature, indian medicinal herbs etc. It is responsive, user friendly, works well with Gutenberg editor and WooCommerce compatible and SEO plugins compatible. Flexible, scalable and easy to use.
 
== Theme Resources == 

Theme is built using the following resource bundles:

* All js that have been used are within folder /js of theme.

*   SKT Themes Self Designed Images:
	skt-ayurveda/images/free-vs-pro.png
	skt-ayurveda/images/icon-skt-templates.png
	skt-ayurveda/images/sktskill.jpg
	skt-ayurveda/images/logo.png
	skt-ayurveda/images/footer-logo.png
	skt-ayurveda/images/phone-icon.png
		
= 	Declaring these self designed images under GPL license version 2.0 =
License URL: http://www.gnu.org/licenses/gpl-2.0.html

*       Details of images used in Screenshot:
		https://www.maxpixel.net/Kitchen-Natural-Green-Lemon-Basil-Spice-Hairy-906141
		https://www.maxpixel.net/Alcohol-Toilet-Paper-Bucket-Gloves-Mop-Soap-Spray-5567843
		https://www.maxpixel.net/Herbal-Mortar-And-Pestle-Natural-Ayurveda-6600173
		https://www.maxpixel.net/Box-Trade-Money-Dollar-Economy-Dollar-Sign-6281064
		https://www.maxpixel.net/Bohemian-Drawing-Plants-Monstera-Boho-Leaves-7379213

		= 	Declaring these self designed images under GPL license version 2.0 =
			License URL: http://www.gnu.org/licenses/gpl-2.0.html
		
		Image used from maxpixel.net
		This site provides images under CC0 license
 		CC0 license: https://creativecommons.org/share-your-work/public-domain/cc0 		
		
*   skt-ayurveda/customize-pro	

		Customize Pro code based on Justintadlock’s Customizer-Pro 
		Source Code URL : https://github.com/justintadlock/trt-customizer-pro			
		License : http://www.gnu.org/licenses/gpl-2.0.html
		Copyright 2016, Justin Tadlock	justintadlock.com

= Dashicons =
* 		License: dashicons is licensed under GPLv2
* 		https://github.com/WordPress/dashicons	
		
= WPTT Webfont Loader =
		https://github.com/WPTT/webfont-loader
		License: Distributed under the terms of the MIT License (https://github.com/WPTT/webfont-loader/blob/master/LICENSE)
		Copyright: 2020 Ari Stathopoulos, WordPress Themes Team	

= Google Font's =		
* 		Poppins - https://fonts.google.com/specimen/Poppins   
* 		License: Distributed under the terms of the SIL Open Font License (OFL) , version 1.1 	https://scripts.sil.org/cms/scripts/page.php?site_id=nrsi&id=OFL

* 		Playfair Display - https://fonts.google.com/specimen/Playfair+Display   
* 		License: Distributed under the terms of the SIL Open Font License (OFL) , version 1.1 	https://scripts.sil.org/cms/scripts/page.php?site_id=nrsi&id=OFL
	
				 
        
For any help you can mail us at support[at]sktthemes.com

== Changelog ==
= 2.1 =
*      Author URL to be rectified.

= 2.0 =
*      Compatible with PHP 8.3
*      Compatible with WordPress 6.9
*      Footer credentials tweaked

= 1.9 =
*      Compatibility check with WordPress version 6.8

= 1.8 =
*      Footer credentials tweaked and Compatibility check with WordPress version 6.7

= 1.7 =
*      Removed footer link.

= 1.6 =  
*      Compatibility check with WordPress version 6.5
*      Footer credentials tweaked

= 1.5 =
*      Compatibility with WordPress 6.4
*      Added company footer credits

= 1.4 =
*      Compatibility with WordPress 6.1
*      Resolved Inner Pages and Post Single UI Issue.

= 1.3 =
*      Resolved pointed issue. 

= 1.2 =
*      Issue resolved as per reviewer comments.

= 1.1 =
*      Changes done as per reviewer comments.

= 1.0 =
*      Initial version release. 		